...
```{r}
entity_donations <- member_payments_db %>% group_by(entity) %>% summarise(total_donations = sum(value)) %>% mutate(overall_total = sum(total_donations)) %>% mutate(share = total_donations/overall_total) %>% arrange(desc(share))

entity_donations

whiters_donations <- member_payments_db %>% filter(entity == "Withers LLP") %>% group_by(name) %>% summarise(total_donations = sum(value))
...
party_payment_db <- left_join(member_payments_db,parties_db %>% 
                                ...
                                rename(party_id = "id",party_name = "name"), by = "party_id")
